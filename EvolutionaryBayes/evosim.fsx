...
let newgen2 (l1: float list, p1) (l2: _ list, p2) =
    ...
    let genome =
        [ for i in order ->
            if bin.Sample()
            then m * (if bin.Sample() then a1.[i] else gene p1)
            else m * (if bin.Sample() then a2.[i] else gene p2) ]
...
let rec drift n avgs (pop: Distribution<_>) =
    if n = 0 then pop, List.rev avgs
        ...
        let pop' =
            [| for _ in 1 .. 500 -> 
                if random.NextDouble() < 0.8 then p1 
                else newgen2 p1 p2 |]

        let t =
            Array.averageBy (fst >> List.sum >> (+) 100.) pop'
...
let newgen l =
    let p = l |> List.sum |> scaleTo 0. 1. -81. 81.
    (flips 27 [] p).Sample()    
...
let m =
    ParticleFilters.PopulationSampler
        (prior = flips 27 [] 0.5, 
         mutate = (fun p m -> newgen (mix (p.Sample()) m)),
         scorer =
             (List.sum
              >> ((+) 100.)
              >> logisticRange 65. 95.))
...
let m2 =
    ParticleFilters.PopulationSampler
        (prior = flips2 27 [] 0.5, 
         mutate = (fun p m -> newgen2 (p.Sample()) m),
         scorer =
             (fst
              >> List.sum
              >> ((+) 100.)
...
let todist =
    ParticleFilters.weightWith (List.sum >> ((+) 100.) >> logisticRange 25. 45.)
    >> categorical2

let todist2 l =
    l
    |> ParticleFilters.weightWith (
        fst
        >> List.sum
        >> ((+) 100.)
        >> logisticRange 25. 45.)
...
let zzx pr g =
    let m =
        ParticleFilters.PopulationSampler
            (prior = pr, 
             mutate = (fun (p:Distribution<_>) m -> newgen (mix (p.Sample()) m)),
             scorer =
                 (List.sum
                  >> ((+) 100.)
...
let zzx2 pr g =
    let m =
        ParticleFilters.PopulationSampler
            (prior = pr, 
             mutate = (fun (p:Distribution<_>) m -> newgen2 (p.Sample()) m),
             scorer =
                 (fst >> List.sum
                  >> ((+) 100.)
                  >> logisticRange 25. 45.))//>> logisticRange 65. 95.))
